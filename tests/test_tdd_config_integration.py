...
class TestSoupIntegration:
    """TDD contracts for TofuSoup integration."""
    
    def test_create_soup_config_prioritizes_soup_toml(self):
        """
        CONTRACT: create_soup_config should prioritize soup.toml for TofuSoup
        """
        with tempfile.TemporaryDirectory() as tmpdir:
...
""")
            ...
            with patch('pathlib.Path.cwd', return_value=pathlib.Path(tmpdir)):
                ...
                # Should have TOFUSOUP_WORKENV_ as highest priority
                assert isinstance(config.sources[0], EnvironmentConfigSource)
                assert config.sources[0].prefix == "TOFUSOUP_WORKENV"
    
    def test_soup_backwards_compatibility(self):
        """
        CONTRACT: TofuSoup should be able to use wrkenv with soup.toml
        """
        with tempfile.TemporaryDirectory() as tmpdir:
            soup_toml = pathlib.Path(tmpdir) / "soup.toml"
            soup_toml.write_text("""
...
""")
            ...
            with patch('pathlib.Path.cwd', return_value=pathlib.Path(tmpdir)):
                # This is how TofuSoup would use it
                config = create_soup_config()
                
                # Should read from soup.toml
                ...
                assert config.get_setting("install_path") == "~/.tofusoup/tools"
    
    def test_tofusoup_can_inject_custom_config(self):
        """
        CONTRACT: TofuSoup should be able to inject its own configuration
        """
        # TofuSoup could create its own config source
        class TofuSoupConfigSource(ConfigSource):
            def __init__(self, soup_config):
                self.soup_config = soup_config
            
            def get_tool_version(self, tool_name: str):
                # Could read from soup's internal config
                return self.soup_config.get("tools", {}).get(tool_name)
        
        # Inject TofuSoup's config as highest priority
        soup_source = TofuSoupConfigSource({"tools": {"terraform": "1.9.0"}})
        config = WorkenvConfig(sources=[soup_source])
        
        assert config.get_tool_version("terraform") == "1.9.0"
