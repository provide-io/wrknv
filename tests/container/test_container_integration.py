...
class TestContainerVolumeIntegration:
    ...
    def container_manager(self, test_config):
        ...
        try:
            subprocess.run(
                ["docker", "stop", manager.CONTAINER_NAME],
                capture_output=True,
                timeout=10
            )
            subprocess.run(
                ["docker", "rm", manager.CONTAINER_NAME],
                capture_output=True,
                timeout=10
            )
        except:
    ...
    def test_shared_downloads_readonly(self, container_manager):
        ...
        cmd = [
            "docker", "exec", container_manager.CONTAINER_NAME,
            "cat", "/downloads/readonly_test.txt"
        ]
        result = subprocess.run(cmd, capture_output=True, text=True)
        assert result.returncode == 0
        ...
        try:
            ...
            for manager in [manager1, manager2]:
                cmd = [
                    ...
                    "cat", "/downloads/shared_resource.txt"
                ]
                result = subprocess.run(cmd, capture_output=True, text=True)
                assert result.returncode == 0
            ...
            cmd1 = [
                "docker", "exec", manager1.CONTAINER_NAME,
                "sh", "-c",
            ...
            cmd2 = [
                "docker", "exec", manager2.CONTAINER_NAME,
                "sh", "-c",
    ...
    def test_large_file_handling(self, container_manager):
        ...
        cmd = [
            "docker", "exec", container_manager.CONTAINER_NAME,
            "dd", "if=/dev/zero", "of=/workspace/large_file.bin",
            "bs=1M", "count=10"
        ]
        result = subprocess.run(cmd, capture_output=True)
        assert result.returncode == 0
