...
class TestContainerConfigCompatibility:
    """Test backward compatibility of ContainerConfig."""
    ...
    def test_old_config_without_new_fields(self):
        ...
        config = ContainerConfig.from_dict(old_config_dict)
        
        assert config.enabled is True
        assert config.base_image == "ubuntu:22.04"
        assert config.storage_path == "~/.wrknv/containers"  # Default
        assert "workspace" in config.persistent_volumes  # Default
        assert len(config.volume_mappings) == 0  # Default empty

    def test_mixed_old_and_new_config(self):
        """Test config with mix of old and new fields."""
        mixed_config = {
            "enabled": True,
            "base_image": "ubuntu:22.04",
            "volumes": ["/old:/style"],  # Old style
            "storage_path": "/new/path",  # New field
            "persistent_volumes": ["workspace"],  # New field
        }
        
        config = ContainerConfig.from_dict(mixed_config)
        
        assert config.volumes == ["/old:/style"]
        assert config.storage_path == "/new/path"
        assert config.persistent_volumes == ["workspace"]