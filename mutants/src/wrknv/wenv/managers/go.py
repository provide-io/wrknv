...
class GoManager(BaseToolManager):
    ...
    def xǁGoManagerǁ_install_from_archive__mutmut_orig(self, archive_path: pathlib.Path, version: str) -> None:
        ...
        try:
            ...
            shutil.move(str(go_root), str(version_dir / "go"))
    ...
    def xǁGoManagerǁ_install_from_archive__mutmut_27(self, archive_path: pathlib.Path, version: str) -> None:
        ...
        try:
            ...
            shutil.move(None, str(version_dir / "go"))
    ...
    def xǁGoManagerǁ_install_from_archive__mutmut_29(self, archive_path: pathlib.Path, version: str) -> None:
        ...
        try:
            ...
            shutil.move(str(version_dir / "go"))
    ...
    def xǁGoManagerǁ_install_from_archive__mutmut_31(self, archive_path: pathlib.Path, version: str) -> None:
        ...
        try:
            ...
            shutil.move(str(None), str(version_dir / "go"))
    ...
    def xǁGoManagerǁ_install_from_archive__mutmut_32(self, archive_path: pathlib.Path, version: str) -> None:
        ...
        try:
            ...
            shutil.move(str(go_root), str(None))
    ...
    def xǁGoManagerǁ_install_from_archive__mutmut_34(self, archive_path: pathlib.Path, version: str) -> None:
        ...
        try:
            ...
            shutil.move(str(go_root), str(version_dir / "XXgoXX"))
    ...
    def xǁGoManagerǁ_install_from_archive__mutmut_68(self, archive_path: pathlib.Path, version: str) -> None:
        """Install Go from downloaded tar.gz archive."""
        # Create version-specific directory
        version_dir = self.install_path / self.tool_name / version
        version_dir.mkdir(parents=True, exist_ok=True)
    ...
    def xǁGoManagerǁ_install_from_archive__mutmut_69(self, archive_path: pathlib.Path, version: str) -> None:
        """Install Go from downloaded tar.gz archive."""
        # Create version-specific directory
        version_dir = self.install_path / self.tool_name / version
        version_dir.mkdir(parents=True, exist_ok=True)
    ...
    'xǁGoManagerǁ_install_from_archive__mutmut_1': xǁGoManagerǁ_install_from_archive__mutmut_1, 
        ...
        'xǁGoManagerǁ_install_from_archive__mutmut_68': xǁGoManagerǁ_install_from_archive__mutmut_68, 
        'xǁGoManagerǁ_install_from_archive__mutmut_69': xǁGoManagerǁ_install_from_archive__mutmut_69
    }
    ...
    def xǁGoManagerǁverify_installation__mutmut_40(self, version: str) -> bool:
        """Verify that Go installation works and version matches."""
        binary_path = self.get_binary_path(version)
        if not binary_path.exists():
            logger.error(f"Go binary not found at {binary_path}")
    ...
    def xǁGoManagerǁverify_installation__mutmut_41(self, version: str) -> bool:
        """Verify that Go installation works and version matches."""
        binary_path = self.get_binary_path(version)
        if not binary_path.exists():
            logger.error(f"Go binary not found at {binary_path}")
    ...
    def xǁGoManagerǁverify_installation__mutmut_42(self, version: str) -> bool:
        """Verify that Go installation works and version matches."""
        binary_path = self.get_binary_path(version)
        if not binary_path.exists():
            logger.error(f"Go binary not found at {binary_path}")
    ...
    def xǁGoManagerǁverify_installation__mutmut_43(self, version: str) -> bool:
        """Verify that Go installation works and version matches."""
        binary_path = self.get_binary_path(version)
        if not binary_path.exists():
            logger.error(f"Go binary not found at {binary_path}")
    ...
    def xǁGoManagerǁverify_installation__mutmut_44(self, version: str) -> bool:
        """Verify that Go installation works and version matches."""
        binary_path = self.get_binary_path(version)
        if not binary_path.exists():
            logger.error(f"Go binary not found at {binary_path}")
    ...
    def xǁGoManagerǁverify_installation__mutmut_45(self, version: str) -> bool:
        """Verify that Go installation works and version matches."""
        binary_path = self.get_binary_path(version)
        if not binary_path.exists():
            logger.error(f"Go binary not found at {binary_path}")
    ...
    def xǁGoManagerǁverify_installation__mutmut_46(self, version: str) -> bool:
        """Verify that Go installation works and version matches."""
        binary_path = self.get_binary_path(version)
        if not binary_path.exists():
            logger.error(f"Go binary not found at {binary_path}")
    ...
    def xǁGoManagerǁverify_installation__mutmut_47(self, version: str) -> bool:
        """Verify that Go installation works and version matches."""
        binary_path = self.get_binary_path(version)
        if not binary_path.exists():
            logger.error(f"Go binary not found at {binary_path}")
    ...
    def xǁGoManagerǁverify_installation__mutmut_48(self, version: str) -> bool:
        """Verify that Go installation works and version matches."""
        binary_path = self.get_binary_path(version)
        if not binary_path.exists():
            logger.error(f"Go binary not found at {binary_path}")
    ...
    'xǁGoManagerǁverify_installation__mutmut_1': xǁGoManagerǁverify_installation__mutmut_1, 
        ...
        'xǁGoManagerǁverify_installation__mutmut_40': xǁGoManagerǁverify_installation__mutmut_40, 
        'xǁGoManagerǁverify_installation__mutmut_41': xǁGoManagerǁverify_installation__mutmut_41, 
        'xǁGoManagerǁverify_installation__mutmut_42': xǁGoManagerǁverify_installation__mutmut_42, 
        'xǁGoManagerǁverify_installation__mutmut_43': xǁGoManagerǁverify_installation__mutmut_43, 
        'xǁGoManagerǁverify_installation__mutmut_44': xǁGoManagerǁverify_installation__mutmut_44, 
        'xǁGoManagerǁverify_installation__mutmut_45': xǁGoManagerǁverify_installation__mutmut_45, 
        'xǁGoManagerǁverify_installation__mutmut_46': xǁGoManagerǁverify_installation__mutmut_46, 
        'xǁGoManagerǁverify_installation__mutmut_47': xǁGoManagerǁverify_installation__mutmut_47, 
        'xǁGoManagerǁverify_installation__mutmut_48': xǁGoManagerǁverify_installation__mutmut_48
    }
