...
def stop_container(config: Optional[WorkenvConfig] = None) -> bool:
    """Stop the development container."""
    manager = ContainerManager(config)
    return manager.stop()


def restart_container(config: Optional[WorkenvConfig] = None) -> bool:
    """Restart the development container."""
    manager = ContainerManager(config)
    return manager.restart()


def container_status(config: Optional[WorkenvConfig] = None) -> None:
...
def clean_container(config: Optional[WorkenvConfig] = None) -> bool:
    """Clean up container and image."""
    manager = ContainerManager(config)
    return manager.clean()


def rebuild_container(config: Optional[WorkenvConfig] = None) -> bool:
