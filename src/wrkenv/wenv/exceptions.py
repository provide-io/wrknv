...
class WrkenvError(Exception):
    """Base exception for all wrkenv errors."""
...
class ConfigurationError(WrkenvError):
    ...
    def __init__(self, message: str, suggestion: str | None = None, line_number: int | None = None):
        if line_number:
...
class ProfileError(WrkenvError):
    ...
    def __init__(self, profile_name: str, message: str = None, available_profiles: list[str] = None):
        if not message:
    ...
    def __init__(self, tool: str, version: str = None, available_versions: list[str] = None):
        ...
        if available_versions:
            ...
            if len(available_versions) > 5:
                versions_str += "..."
            suggestion = f"Available versions: {versions_str}. Use 'wrkenv {tool} --list' to see all"
...
class WorkenvError(WrkenvError):
    ...
    def __init__(self, message: str, workenv_path: str = None):
        suggestion = None
        if workenv_path:
            suggestion = f"Try recreating the workenv: rm -rf {workenv_path} && wrkenv setup --init"
