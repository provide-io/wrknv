...
class WorkspaceSync:
    ...
    def _generate_configs(self, repo: RepoConfig, context: dict[str, Any]) -> dict[str, str]:
        ...
        # Always generate pyproject.toml
        if "pyproject" in repo.features or not repo.features:
            configs["pyproject.toml"] = self.template_generator.generate_pyproject(context)

        # Generate CLAUDE.md if requested
        if "claude" in repo.features:
            configs["CLAUDE.md"] = self.template_generator.generate_claude_md(context)

        # Generate .gitignore if requested
        if "gitignore" in repo.features:
            configs[".gitignore"] = self.template_generator.generate_gitignore(context)
