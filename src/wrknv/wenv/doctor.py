...
class WrknvDoctor:
    ...
    def _check_wrknv_installation(self):
    ...
    def _check_workenv_structure(self):
    ...
    def _check_env_script(self):
        ...
        if not is_wrknv_generated:
            self.checks_failed.append((
                "env.sh", 
                "Not generated by wrknv - run 'wrknv generate' to regenerate"
            ))
            return
    ...
    def _test_env_script_execution(self, env_script: Path):
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                self.checks_failed.append((
                    "env.sh Execution",
                    "VIRTUAL_ENV not set. Regenerate with 'wrknv generate'"
                ))
            else:
            ...
            self.checks_failed.append((
                "env.sh Execution",
                f"Unexpected error: {e}. This may be a bug in wrknv"
            ))
    ...
    def _check_sibling_packages(self):
    ...
    def _check_common_issues(self):
        ...
        if (Path.cwd() / ".venv").exists():
            self.checks_warning.append((
                ".venv Directory",
                "Found - should use workenv/ instead with wrknv"
            ))
        ...
        if active_vars:
            self.checks_warning.append((
                ...
                f"Potentially conflicting: {', '.join(active_vars)}"
            ))
        ...
        if sys.prefix != sys.base_prefix:
            self.checks_warning.append((
                "Virtual Environment",
                "Already activated - may conflict with wrknv"
            ))
    ...
    def _print_recommendations(self):
        ...
        if env_failure_count >= 3:
            ...
            self.console.print(
                "  [dim]Please report at: https://github.com/provide-io/wrknv/issues[/dim]"
            )
