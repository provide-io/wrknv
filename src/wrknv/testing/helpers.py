...
def activate_workenv(package_name: Optional[str] = None) -> Dict[str, str]:
    """
    ...
    if not workenv_dir.exists():
        raise RuntimeError(
            ...
            f"Run 'wrknv setup' or 'source env.sh' to create it."
        )
    ...
    site_packages = workenv_dir / "lib" / f"python{sys.version_info.major}.{sys.version_info.minor}" / "site-packages"
...
def workenv_context(package_name: Optional[str] = None):
    """
...
class WorkenvTestRunner:
    ...
    def setup(self, force: bool = False) -> None:
        ...
        if not env_script.exists():
            raise RuntimeError(
                f"env.sh not found in {Path.cwd()}\n"
                f"Run 'wrknv generate' to create it."
            )
    ...
    def install_deps(self, editable: bool = True, extras: Optional[str] = None) -> None:
        """
        ...
        cmd = [
            str(self.workenv_dir / "bin" / "python"),
            "-m", "pip", "install"
        ]
